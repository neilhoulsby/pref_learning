...
computePreferenceKernel <- function(itemFeaturesA, itemFeaturesB, lengthScale) {

	n <- nrow(itemFeaturesA)

	# We compute the kernel matrix

	QA <- matrix(apply(itemFeaturesA^2, 1, sum), n, n)
	distanceA <- QA + t(QA) - 2 * itemFeaturesA %*% t(itemFeaturesA)
	KA <- exp(-0.5 * distanceA / lengthScale^2)

	QB <- matrix(apply(itemFeaturesB^2, 1, sum), n, n)
	distanceB <- QB + t(QB) - 2 * itemFeaturesB %*% t(itemFeaturesB)
	KB <- exp(-0.5 * distanceB / lengthScale^2)

	distanceAB <- QA + t(QB) - 2 * itemFeaturesA %*% t(itemFeaturesB)
	KAB <- exp(-0.5 * distanceAB / lengthScale^2)

	distanceBA <- QB + t(QA) - 2 * itemFeaturesB %*% t(itemFeaturesA)
...
##
# Function that computes the kernel matrix for the training item pairs and the new item pairs.
#

updatePreferenceKernelMatrix <- function(itemFeaturesA, itemFeaturesB, itemFeaturesAnew, itemFeaturesBnew, lengthScale) {

	# We obtain the features of the different items

	n <- nrow(itemFeaturesAnew)
	m <- nrow(itemFeaturesA)

	# We compute the kernel matrix

	QAnew <- matrix(apply(itemFeaturesAnew^2, 1, sum), m, n, byrow = T)
	QBnew <- matrix(apply(itemFeaturesBnew^2, 1, sum), m, n, byrow = T)

	QA <- matrix(apply(itemFeaturesA^2, 1, sum), m, n)
	QB <- matrix(apply(itemFeaturesB^2, 1, sum), m, n)

	distanceA <- QA + QAnew - 2 * itemFeaturesA %*% t(itemFeaturesAnew)
	KA <- exp(-0.5 * distanceA / lengthScale^2)

	distanceB <- QB + QBnew - 2 * itemFeaturesB %*% t(itemFeaturesBnew)
	KB <- exp(-0.5 * distanceB / lengthScale^2)

	distanceAB <- QA + QBnew - 2 * itemFeaturesA %*% t(itemFeaturesBnew)
	KAB <- exp(-0.5 * distanceAB / lengthScale^2)

	distanceBA <- QB + QAnew - 2 * itemFeaturesB %*% t(itemFeaturesAnew)
	KBA <- exp(-0.5 * distanceBA / lengthScale^2)
